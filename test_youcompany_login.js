...
async function testYouCompanyLogin() {
  ...
  for (const userData of testUsers) {
    ...
    try {
      ...
      console.log(`   Hash calculé: ${passwordHash}`);
      console.log(`   Hash en DB: ${user.encrypted_password}`);
      ...
      if (user.encrypted_password === passwordHash) {
        ...
        console.error(`   Hash attendu: ${passwordHash}`);
        console.error(`   Hash trouvé: ${user.encrypted_password}`);
      }
    } catch (error) {
