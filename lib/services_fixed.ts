...
export const financialServiceFixed = {
  ...
  async getFinancialStats(partnerId?: string) {
    ...
    const stats = {
      total_debloque: data.filter(t => t.type === 'debloque' && t.statut === 'Validé').reduce((sum, t) => sum + t.montant, 0),
      total_recupere: data.filter(t => t.type === 'recupere' && t.statut === 'Validé').reduce((sum, t) => sum + t.montant, 0),
      total_commissions: data.filter(t => t.type === 'commission' && t.statut === 'Validé').reduce((sum, t) => sum + t.montant, 0),
      total_transactions: data.length,
      montant_moyen: data.length > 0 ? data.reduce((sum, t) => sum + t.montant, 0) / data.length : 0
    };
