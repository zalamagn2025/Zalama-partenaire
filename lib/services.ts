...
export const financialService = {
  ...
  calculateAdvancedStats(transactions: any[]) {
    ...
    if (transactions.length > 0) {
      ...
      // Calculer la balance
      stats.balance =
        stats.total_debloque -
        stats.total_recupere +
        stats.total_revenus -
      ...
      stats.repartition_par_statut =
        this.calculateStatusDistribution(transactions);
    }
...
export const messageService = {
    ...
    if (partnerId) {
      ...
      if (employees && employees.length > 0) {
        ...
        if (users && users.length > 0) {
          ...
          query = query.or(
            `expediteur_id.in.(${userIds.join(
              ","
            )}),destinataire_id.in.(${userIds.join(",")})`
          );
        } else {
  ...
  async sendReply(replyData: any) {
    const { data, error } = await supabase.from("messages").insert(replyData);
    return { data, error };
  },
};
...
export const demandeService = {
  ...
  async createDemande(
    demandeData: Omit<Demande, "id" | "created_at" | "updated_at">
  ) {
    const { data, error } = await supabase
      .from("demandes")
      .insert(demandeData)
...
export const partnershipRequestService = {
  ...
  async validateRequest(requestId: string, type: string, secteur: string) {
    const { data, error } = await supabase.rpc("validate_partnership_request", {
...
export const demandeAvanceService = {
  ...
  async getDemandesAvance(partnerId: string) {
    ...
    if (error) {
      console.error(
        "Erreur lors de la récupération des demandes d'avance:",
        error
      );
      return { data: null, error };
    }
  ...
  async updateDemandeAvance(
    id: string,
    updates: Partial<SalaryAdvanceRequest>
  ) {
    const { data, error } = await supabase
      .from("salary_advance_requests")
      .update(updates)
...
export class PartnerDataService {
  ...
  async getRemboursements(): Promise<any[]> {
    try {
      const { data, error } = await supabase
        .from("remboursements")
        .select(
  ...
  async getFinancialStats() {
    try {
      const remboursements = await this.getRemboursements();

      // Utiliser le service de remboursements pour calculer les statistiques
      const stats = remboursementService.calculateAdvancedStats(remboursements);
      ...
      console.error(
        "Erreur lors du calcul des statistiques financières:",
        error
      );
      return {
        total_debloque: 0,
  ...
  async getPartnerStats() {
    try {
      const [employees, remboursements, alerts, avis, demandes] =
        await Promise.all([
          this.getEmployees(),
      ...
      const totalAmount = remboursements.reduce((sum, remboursement) => {
        return sum + (remboursement.montant_total_remboursement || 0);
      }, 0);

      // Calculer les demandes en cours
      const pendingDemandes = demandes.filter(
        (d) => d.statut === "En attente"
      ).length;

      // Calculer les avis positifs
      const positiveAvis = avis.filter(
        (a) => a.type_retour === "positif"
      ).length;
      const averageRating =
        avis.length > 0
  ...
  async getChartData() {
    try {
      const [remboursements, demandes] = await Promise.all([
        this.getRemboursements(),
        this.getSalaryAdvanceRequests(),
      ]);

      // Données mensuelles pour les remboursements
      const monthlyTransactions = this.groupByMonth(
        remboursements,
        "date_creation"
      );
      ...
      // Répartition par statut de remboursement
      const transactionTypes = this.groupByType(remboursements, "statut");
...
export class NotificationService {
  ...
  private static mapAlertTypeToNotificationType(
    alertType: string
