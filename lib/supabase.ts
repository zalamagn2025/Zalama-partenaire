...
const supabaseAnonKey =
  ...
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1zcG1yemxxaHdwZGtrYnVyaml3Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTA3ODcyNTgsImV4cCI6MjA2NjM2MzI1OH0.zr-TRpKjGJjW0nRtsyPcCLy4Us-c5tOGX71k5_3JJd0";
...
export const createTestUsers = async () => {
  ...
  for (const userData of testUsers) {
    try {
      ...
      if (userError) {
        console.error(
          `Erreur lors de l'insertion des données utilisateur pour ${userData.email}:`,
          userError
        );
        console.error(
          "Détails de l'erreur:",
      ...
      const { error: partnerError } = await supabase.from("partners").insert({
        ...
        activity_domain:
          userData.organisation === "InnovaTech" ? "Technologie" : "Éducation",
      ...
      if (partnerError) {
        console.error(
          `Erreur lors de la création du partenaire pour ${userData.email}:`,
          partnerError
        );
        console.error(
          "Détails de l'erreur partenaire:",
      ...
      console.log(
        `Utilisateur créé avec succès: ${userData.email} (ID: ${userId})`
      );
    } catch (error) {
...
export async function createTestData() {
  try {
    ...
    console.log(
      `${users.length} utilisateurs prêts pour la création des partenaires...`
    );
    ...
    console.log(
      "Employés récupérés pour création des données:",
        ...
        if (employee.email) {
          ...
          if (userData) {
            ...
            demandes.push({
              ...
              type_demande:
                typesDemande[Math.floor(Math.random() * typesDemande.length)],
              description:
                descriptions[Math.floor(Math.random() * descriptions.length)],
              statut: ["En attente", "En cours", "Approuvée"][
    ...
    const alerts = [
      {
        ...
        description:
          "Une nouvelle demande de partenariat a été soumise et nécessite votre attention.",
        type: "Importante",
        statut: "Nouvelle",
        source: "Système",
        priorite: 2,
      },
      {
        ...
        description:
          "Une maintenance est prévue ce weekend pour améliorer les performances.",
        type: "Information",
        statut: "En cours",
        source: "Système",
        priorite: 1,
      },
      {
        ...
        description:
          "Un paiement est en retard et nécessite une action immédiate.",
        type: "Critique",
        statut: "Nouvelle",
        source: "Finance",
        priorite: 3,
    ...
    if (employeesData && employeesData.length > 0) {
      ...
      for (const employee of employeesData) {
        ...
        demandesAvance.push({
          ...
          date_demande: new Date().toISOString(),
          date_remboursement: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)
            .toISOString()
            .split("T")[0], // +30 jours
          statut: ["En attente", "En cours", "Approuvée"][
            Math.floor(Math.random() * 3)
      ...
      console.log(
        "Création des demandes d'avance avec les données:",
        demandesAvance
      );
      ...
      for (const demande of demandesAvance) {
        ...
        if (error) {
          console.error(
            "Erreur lors de la création de la demande d'avance:",
            error
          );
    ...
    if (employeesData && employeesData.length > 0) {
      ...
      for (const employee of employeesData) {
        ...
        demandesP2P.push({
          ...
          date_demande: new Date().toISOString(),
          date_debut_remboursement: new Date(
...
// Interface pour la table remboursements
...
// Type étendu pour les remboursements avec les données des employés
export interface RemboursementWithEmployee extends Remboursement {
  employees?: Employee;
