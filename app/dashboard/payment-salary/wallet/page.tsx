...
export default function PaymentSalaryPage() {
  ...
  const loadPaymentData = async (month?: number, year?: number) => {
    ...
    try {
      ...
      // TODO: Migrer vers le nouveau backend
      // Utiliser l'API pour récupérer les employés avec leurs informations de paiement
      const apiUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL || 'https://sandbox.zalamagn.com'}/partner-payments/employees`;
      const params = new URLSearchParams();
      if (month) params.append('mois', String(month));
      if (year) params.append('annee', String(year));
      ...
      const responseFormatted = {
        success: true,
        data: {
          partenaire: { nom: session.partner?.companyName || 'Partenaire' },
          periode: { mois: month, annee: year },
          employes: Array.isArray(data) ? data : (data.data || []),
          statistiques: {},
        },
      };
      ...
      setPaymentData(responseFormatted);
      setSelectedEmployees(new Set()); // Réinitialiser la sélection

      console.log("✅ Données de paiement chargées:", responseFormatted);
  ...
  const executePayments = async () => {
    ...
    try {
      ...
      // TODO: Migrer vers le nouveau backend
      // Utiliser l'API pour traiter les paiements en batch
      const apiUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL || 'https://sandbox.zalamagn.com'}/partner-payments/batch/process-wallet`;
      ...
      const responseFormatted = {
        success: true,
        data: data.data || data,
        message: data.message || 'Paiements exécutés avec succès',
      };

      console.log("📊 Réponse complète de l'API:", responseFormatted);

      // Gérer les erreurs détaillées
      if (!responseFormatted.success) {
        setPaymentError(responseFormatted);

        // Afficher les erreurs spécifiques
        if (
          (responseFormatted.data as any)?.erreurs &&
          (responseFormatted.data as any).erreurs.length > 0
        ) {
          const errors = (responseFormatted.data as any).erreurs;
      ...
      console.log("✅ Paiements exécutés avec succès:", responseFormatted);

      // Afficher les statistiques de succès
      const stats = (responseFormatted.data as any)?.statistiques;
      if (stats) {
        toast.success(
        ...
        toast.success(
          `Paiements exécutés avec succès pour ${
            responseFormatted.data?.nombre_employes || selectedEmployees.size
          } employés`
        );
      }
